                    ...
                    }) = arguments
                    {
                        ...
                        }))) = args.first()
                        {
                            if let Some(PathSegment { ident, .. }) = segments.first() {
                                return match ident.to_string().as_str() {
                                    "Box" => quote!(Convert::convert_option_box),
                                    "Vec" => quote!(Convert::convert_option_vec),
                                    _ => quote!(Convert::convert_option),
                                };
                            }
                        }
                    }
                    quote!(Convert::convert_option)
                }
                "Vec" => {
                    if let PathArguments::AngleBracketed(AngleBracketedGenericArguments {
                        args,
                    ...
                    }) = arguments
                    {
                        ...
                        }))) = args.first()
                        {
                            if let Some(PathSegment { ident, .. }) = segments.first() {
                                return match ident.to_string().as_str() {
                                    "Vec" => quote!(Convert::convert_matrix),
                                    "Box" => quote!(Convert::convert_vec_box),
                                    _ => quote!(Convert::convert_vec),
                                };
                            }
                        }
                    }
                    quote!(Convert::convert_vec)
                }
                "Box" => quote!(Convert::convert_box),
                _ => quote!(Convert::convert),
            };
        }
    }
        ...
        for field in variants {
            ...
            if let Fields::Named(FieldsNamed { named, .. }) = &field.fields {
                ...
                for field in named {
                    let field_name = field.ident.clone().unwrap();
                    let var_type = get_var_type(&field.ty);
                    inner_fields.push(quote!(#field_name,));
                    codes.push(quote! {
