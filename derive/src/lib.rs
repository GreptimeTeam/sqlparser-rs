...
fn get_var_type(ty: &Type) -> TokenStream {
    let span = ty.span();
    if let Type::Path(TypePath {
        path: Path { segments, .. },
        ..
...
fn get_crate_path(st: &DeriveInput) -> TokenStream {
    let span = st.span();
...
fn convert_struct(st: &DeriveInput) -> TokenStream {
    let name = &st.ident;
    ...
    {
        let span = named.span();
        let mut fields: Vec<TokenStream> = Vec::with_capacity(named.len());
        for field in named {
    ...
    }) = &st.data
    {
        ...
        let mut fields: Vec<TokenStream> = Vec::with_capacity(unnamed.len());
...
fn convert_enum(st: &DeriveInput) -> TokenStream {
    let name = &st.ident;
    let path = get_crate_path(st);
    if let Data::Enum(DataEnum { variants, .. }) = &st.data {
        let span = variants.span();
        let mut fields: Vec<TokenStream> = Vec::with_capacity(variants.len());
        for field in variants {
            ...
            if let Fields::Unnamed(FieldsUnnamed { unnamed, .. }) = &field.fields {
                let inner_names = ('a'..='z')
                    ...
                    .to_vec();
                let mut codes: Vec<TokenStream> = Vec::with_capacity(unnamed.len());
            ...
            if let Fields::Named(FieldsNamed { named, .. }) = &field.fields {
                let mut inner_fields: Vec<TokenStream> = Vec::with_capacity(named.len());
                let mut codes: Vec<TokenStream> = Vec::with_capacity(named.len());
                let span = named.span();
                for field in named {
                    let field_name = field.ident.clone().unwrap();
                    let span = field_name.span();
...
fn convert_union(st: &DeriveInput) -> TokenStream {
    let name = &st.ident;
    let path = get_crate_path(st);

    if let Data::Union(data_union) = &st.data {
        let span = data_union.fields.span();
        let mut fields: Vec<TokenStream> = Vec::with_capacity(data_union.fields.named.len());
...
fn expand_df_convert(st: &DeriveInput) -> TokenStream {
    match st.data {
        Data::Struct(_) => convert_struct(st),
        Data::Enum(_) => convert_enum(st),
        Data::Union(_) => convert_union(st),
    }
}
